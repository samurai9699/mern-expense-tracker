...
const TransactionPage = () => {
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		const amount = parseFloat(formData.amount); // convert amount to number bc by default it is string  // and the reason it's coming from an input field
		try{
	...
	return (
		<div className='h-screen max-w-4xl mx-auto flex flex-col items-center'>
			...
			<form className='w-full max-w-lg flex flex-col gap-5 px-3 ' onSubmit={handleSubmit}>
				...
				>
					{loadingUpdate ? "Updating..." : "Track Transaction"}
				</button>
			</form>
		</div>
	);
